...
const PaymentItem = ({ payment, handleChange, selected = false }) => {
    ...
    const [optimisticIsSelected, setOptimisticIsSelected] = useOptimistic(selected, (_, newState) => newState);
    ...
    return (
        <div className={'mt-4'}>
            <label htmlFor={`payment-${payment.id}`} className={'flex items-start justify-start gap-4'}>
                <RadioInput
                    inputProps={{
        ...
        <span className="font-bold text-font inline-block mb-1">
          {payment?.name} {' '}
                    ...
                    <p className="text-sm text-grey3" dangerouslySetInnerHTML={{__html:payment?.description}}/>
                </div>
    ...
    return (
        <div>
            <h3 className="text-lg mb-3">
                {t('cart_form.payment.title')}
            </h3>
            {
                isEmpty(options)
                    ? <p>{t(cart?.delivery === null ? 'payment_select.select_delivery' : 'payment_select.empty')}</p>
                    : (<>
                            {map(options, (payment, i) => {
                                return <PaymentItem key={`payment-item-${i}`} payment={payment} selected={selected?.id === payment.id} handleChange={handleChange} />
                            })}
                        </>
                    )
            }
