...
const ShippingItem = ({ delivery, countries, handleChange, selected = false, children }) => {
    const { cart } = useCart()
    ...
    return (
        <div className={'mt-4'}>
            <div>
                <label htmlFor={`delivery-${delivery.id}`} className={'flex items-start justify-start gap-4'}>
                    ...
                    <div className={'flex-grow'}>
                        ...
                        <p className={'text-sm text-grey3'} dangerouslySetInnerHTML={{ __html: delivery?.description }} />
                    </div>
            ...
            <DeliveryProvider delivery={delivery} selected={optimisticIsSelected} country={find(countries, {id: cart?.countryId})} />
            {children}
        </div>
    );
}
